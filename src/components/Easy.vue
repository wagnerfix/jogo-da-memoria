...
export default {
	...
	created() {
		this.$on('startGameEasy', this.loadGame())
	},
	data() {
		return {
			itens: [],
	...
	methods: {
		loadGame: function () {
			this.itens = []
			let id = 0			
			let list1 = [1, 2, 3, 4, 5, 6].sort(() => Math.random() - 0.5)
			let list2 = [1, 2, 3, 4, 5, 6].sort(() => Math.random() - 0.5)
		...
		cpuPlayer() {
			if (!this.finished) {
				setTimeout(() => {
					...
					let partSelected2 = findOnly1.sort(() => Math.random() - 0.5)
